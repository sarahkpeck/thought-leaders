...
class OptimizePress_Assets_Core {
	...
	function providerItems($type = null, $return = null, $special = null)
	{
		...
		try {
			...
			$data['lists'] = array('error' => array('name' => __('There seems to be an error with the integration data.', OP_SN)));
		}
	...
	function liveSearch()
	{
		...
		if (!empty($results)) {
			...
			echo '<li class="op-live-search-results-item op-live-search-results-item--empty">' . __('No results', OP_SN) . '</li>';
		}
	...
	function parse_assets(){
		if(count($this->parse_list) == 0){
			$assets = array();
			$assets = apply_filters('op_assets_parse_list',$assets);
			$this->parse_list = array_filter($assets);
		}
	...
	function _process_files($folders,$type='style',$returnfiles=false){
		...
		foreach($folders as $f){
			if(isset($f['group']) && isset($f['tag']) && isset($f['folder'])){
				switch($f['group']){
					case 'addon':
					...
					case 'theme':
						...
						break;
					default:
						$path = OP_ASSETS.'images/';
						$url = OP_ASSETS_URL.'images/';
						break;
				}
