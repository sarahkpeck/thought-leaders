...
class OptimizePress_LiveEditor_Assets {
    ...
    function membership_page_listings($atts)
    {
        ...
        $html .= '<script type="text/javascript">(function($) {
            ...
            function resizeWindow(e, currentContainer) {

                var $pagelistingContainers = $containers;

                // On small screen sizes there is no need for calculations
                if ($(window).width() < 767)  {
                    return false;
                }

                if (currentContainer) {
                    // If we are closing the content toggle element we do not need to recalculate the sizes
                    if ($(currentContainer).hasClass("panel-open")) {
                        return false;
                    }
                    $pagelistingContainers = $(currentContainer).find(".element-container");
                }

                $($pagelistingContainers).each(function() {
                    var e = 0;
                    var t = $(this).find("[class*=\'pagelisting-style-\']");

                    t.css({ height: "auto" });

                    /**
                     * We can accurately and safely get the height only if
                     * setting height:auto is executed before getting the height
                     */
                    setTimeout(function () {
                        t.each(function() {
                            var t = $(this).height();
            ...
            $(document).ready(function() {
                ...
                $(window).on("content-toggle", function (e, data) {
                    resizeWindow(e, data);
                });
            });
        })(opjq);</script>' . "\n";
