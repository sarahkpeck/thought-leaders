            ...
            for (var i in attributes) {
                switch (defaults[i].type) {
                    ...
                    case 'font':
                        ...
                        $('#op_asset_browser_slide3 .op-settings-core-button').trigger({type: 'update_button_preview', tag: 'button', id: defaults[i].selector.substr(1), value: $item.attr('alt'), font_type: $item.attr('data-type'), font_family: $item.attr('data-family')});
                        break;
                    case 'slider':
