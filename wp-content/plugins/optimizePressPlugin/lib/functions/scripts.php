...
function op_default_localize(){
    ...
    return $default = array(
        ...
        'flowplayerHTML5' => apply_filters('flowplayer_license', OP_MOD_URL.'blog/video/flowplayer-5.5.0/flowplayer.swf', 'swf_file'),
...
function op_localize_script($types=''){
    ...
    wp_localize_script( OP_SN.'-noconflict-js', 'OptimizePress', apply_filters(OP_SN.'-script-localize',$localize) );
}
    ...
    if(!isset($typography_elements)){
        $typography_elements = array(
            'font_elements' => array(
                ...
                'h1,.main-content h1,.single-post-content h1,.full-width.featured-panel h1,.latest-post .the-title' => 'h1',
...
{
    //Flowplayer (video)
    wp_enqueue_script(OP_SN.'flowplayerhtml5', apply_filters('flowplayer_license', OP_MOD_URL.'blog/video/flowplayer-5.5.0/flowplayer.min.js', 'js_file'), array(OP_SN.'-noconflict-js'), OP_VERSION);
    wp_enqueue_style(OP_SN.'flowplayerminimalist', OP_MOD_URL.'blog/video/flowplayer-5.5.0/skin/minimalist'.OP_SCRIPT_DEBUG.'.css', false, OP_VERSION);
