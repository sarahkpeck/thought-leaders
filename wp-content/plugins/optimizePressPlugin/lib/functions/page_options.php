...
class OptimizePress_Page_Options {
                                    ...
                                    if(preg_match('/'.op_shortcode_regex('op_liveeditor_elements').'/s',$sc,$matches) || $op_popup_present){
                                        ...
                                        if ($op_popup_present) {
                                            ...
                                            foreach ($popup_elements[0] as $popup_element) {
                                                ...
                                                $new_popup_elements .= '<div class="element-container op-popup-element-container ' . $popup_data_style['advancedClass'] . $popup_data_style['hideClasses'] . '"' . $popup_data_fade . '>' . $popup_data_style['elemBefore'] . $popup_element . $popup_data_style['elemAfter'] . '</div>';
                                            }
    ...
    function update_layout($layout,$type='body'){
        ...
        if (null == $entry) {
            ...
            // actually saving only if changes are present and autosave is turned on
            if ($layout !== $entry->layout && defined('OP_AUTOSAVE_ENABLED') && 'Y' == OP_AUTOSAVE_ENABLED) {
     ...
     * @param [string] $matches - op_liveeditor_elements shortcode
