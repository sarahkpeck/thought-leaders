...
if(!function_exists('op_define_vars')){
    ...
    */
    ...
    function checkS3Update($transient)
    {
        ...
        || !isset($_GET['action']) || !in_array($_GET['action'], array('upgrade-plugin', 'update-selected'))
        || !((isset($_GET['plugin']) && $_GET['plugin'] === OP_PLUGIN_SLUG) || (isset($_GET['plugins']) && false !== strpos($_GET['plugins'], OP_PLUGIN_SLUG)))) {
            return $transient;
        }
    ...
    {
        /*
         * Not a LE page, let's leave this joint
         */
        if (false === is_le_page()) {
            return;
        }

        $js         = op_get_option('op_external_plugins_js');
        $themeJs    = (int) op_get_option('op_external_theme_js');
    ...
    function opRemoveStyles()
    {
        ...
        $css        = op_get_option('op_external_plugins_css');
        $themeCss   = (int) op_get_option('op_external_theme_css');
